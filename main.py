...
def main() -> None:
    game_running = True
    reachable_cells_by_pawn = None # Will contain the reachable cells by the pawn
    while game_running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: # Check if the user click on the window
                for cell in board.board:
                    ...
                    if cell["cell_gui"].collidepoint(mouse_position) and not cell["cell_is_empty"]:
                        # Get informations about the pawn that is currently on cell that the user clicked
                        pawn_informations: tuple[dict, int] = player1.get_pawn(player1.player_pawns, list(cell["cell_index"]))
                        if None not in pawn_informations: # If the pawn exists
                            if pawn_informations[0]["pawn_status"] == "alive": # Check if the pawn is alive
